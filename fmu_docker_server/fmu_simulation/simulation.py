...
class Simulation:
    ...
    def update(self):
        ...
        if status == pyfmi.fmi.FMI_OK:
            ...
            self.datamap.update({'time' : {"value":-99999, "unit":"error FMU update"}})
    ...
    def get_update_output(self, time=False, noise=0.0):
        data_dict = dict()
        if time:
            data_dict.update({"time":self.time})
            
        for key in self.dataset_out:
            value = self.datamap[key]["value"]* (1 + random.uniform(-noise, noise))
            data_dict.update({key:value})
        
        return data_dict
    
    def get_update_output_init_dict(self, time=False):
        data_dict = dict()
        if time:
            data_dict.update({"time":copy.deepcopy(list())})
            
        for key in self.dataset_out:
            data_dict.update({key:copy.deepcopy(list())})
        
        return data_dict
    
    def get_input_latest_time_entry(self, rest_time):
        return self.input_config[-1]["time"] + rest_time
    
    def get_input(self, states=list()):
        input = copy.deepcopy(self.input)
        if len(states) > 0:
            input_extracted = {k: input[k] for k in states if k in input}
            return input_extracted
        else:
            return input